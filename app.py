...
app.secret_key = os.environ.get('SECRET_KEY', 'devmgmt-change-me-in-production')
...
def init_db():
    ...
    # Add customer_phone column if missing (migration for older DBs)
    try:
        db.execute("ALTER TABLE devices ADD COLUMN customer_phone TEXT")
    ...
    if row[0] == 0:
        db.execute(
            ...
            (generate_id(), 'admin', generate_password_hash('admin123'), 'admin')
        )
...
def normal_user_filter():
    """Return a WHERE clause and params to restrict normal users to repair-related devices."""
    if session.get('role') == 'normal':
        return (" AND d.status IN ('PENDING_INSPECTION','IN_REPAIR','DAMAGED')", [])
    return ("", [])
...
def api_dashboard():
    db = get_db()
    nf_clause, nf_params = normal_user_filter()
    base_query = f"SELECT COUNT(*) FROM devices d WHERE 1=1{nf_clause}"
    total = db.execute(base_query, nf_params).fetchone()[0]
    customer = db.execute(f"{base_query} AND d.category='CUSTOMER'", nf_params).fetchone()[0]
    for_sale = db.execute(f"{base_query} AND d.category='FOR_SALE'", nf_params).fetchone()[0]
    internal = db.execute(f"{base_query} AND d.category='INTERNAL'", nf_params).fetchone()[0]
    damaged = db.execute(f"{base_query} AND d.status='DAMAGED'", nf_params).fetchone()[0]
    in_repair = db.execute(f"{base_query} AND d.status='IN_REPAIR'", nf_params).fetchone()[0]
    ready = db.execute(f"{base_query} AND d.status='READY'", nf_params).fetchone()[0]
    recent = db.execute(
        f"SELECT id, name, category, status, created_at FROM devices d WHERE 1=1{nf_clause} ORDER BY d.created_at DESC LIMIT 10",
        nf_params
    ).fetchall()
...
def api_devices_list():
    db = get_db()
    nf_clause, nf_params = normal_user_filter()
    query = f"SELECT d.*, (SELECT COUNT(*) FROM repair_records WHERE device_id=d.id) as repairs_count, (SELECT COUNT(*) FROM activity_logs WHERE device_id=d.id) as activities_count FROM devices d WHERE 1=1{nf_clause}"
    params = list(nf_params)
...
def api_devices_create():
    ...
    db.execute("""
        INSERT INTO devices (id, name, serial_number, customer_name, customer_phone, category, sub_category, status,
...
def api_device_update(device_id):
    ...
    fields = ['name', 'serial_number', 'customer_name', 'customer_phone', 'category', 'sub_category', 'status',
...
def api_customer_systems():
    ...
    devices = db.execute(f"""
        ...
        FROM devices d WHERE d.category='CUSTOMER'{nf_clause} ORDER BY d.created_at DESC
    """, nf_params).fetchall()
    return jsonify([dict(d) for d in devices])
...
def api_for_sale_systems():
    ...
    devices = db.execute(f"""
        SELECT d.*, (SELECT COUNT(*) FROM repair_records WHERE device_id=d.id) as repairs_count,
               (SELECT COUNT(*) FROM activity_logs WHERE device_id=d.id) as activities_count
        FROM devices d WHERE d.category='FOR_SALE'{nf_clause} ORDER BY d.created_at DESC
    """, nf_params).fetchall()
    return jsonify([dict(d) for d in devices])
...
def api_internal_systems():
    ...
    nf_clause, nf_params = normal_user_filter()
    sub_category = request.args.get('subCategory')
    cond = f" AND d.sub_category=?" if sub_category else ""
    params = nf_params + ([sub_category] if sub_category else [])
    devices = db.execute(f"""
        SELECT d.*, (SELECT COUNT(*) FROM repair_records WHERE device_id=d.id) as repairs_count,
               (SELECT COUNT(*) FROM activity_logs WHERE device_id=d.id) as activities_count
        FROM devices d WHERE d.category='INTERNAL'{nf_clause}{cond} ORDER BY d.created_at DESC
    """, params).fetchall()
    return jsonify([dict(d) for d in devices])
